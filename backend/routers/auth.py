...
from backend.schemas import AuthAccount, AuthLoginInput, AuthLookupInput, AuthUpsertInput
from backend.services.auth_store import auth_store

router = APIRouter()


@router.post("/api/auth/lookup", response_model=AuthAccount)
...
@router.post("/api/auth/login", response_model=AuthAccount)
...
@router.post("/api/auth/upsert", response_model=AuthAccount)
